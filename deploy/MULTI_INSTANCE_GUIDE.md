...
### 防线 5：检索分块 Prompt 放行上限与大模型表格保护（CHAT_CHUNK_MAX_CHARS）
- **根因场景**：若未配置放行上限，系统早期曾默认回退到 1000 字符硬截断，导致大表格（如包含几十支队伍或长列表）在送入大模型 Prompt 时被强行切断，造成大模型“视野受限”而给出错误统计数据。
- **自动防线**：
  1. 代码层兜底：`apps/api/src/chat/chat.service.ts` 默认后备值统一提升至 `6000` 字符，即使新实例未配置环境变量，也不会退回 1000。
  2. 智能截断保护：内置 `smartTruncateChunkText`，遇到超长表格截断时在完整表格行末尾截断并补齐 Markdown 闭合标记，严禁在行中撕裂语法。
  3. 环境变量支持：新开实例配置文件中显式注入 `CHAT_CHUNK_MAX_CHARS=6000`，支持按业务场景灵活调优。
