...
describe('PermissionService', () => {
  ...
  it('should calculate visible knowledge bases correctly', async () => {
    ...
    // 2. Mock 个人库
    mockPrisma.knowledgeBase.findMany.mockResolvedValueOnce([{ id: 'kb-personal-1' }]);
    
    // 3. Mock 组织继承
    mockPrisma.userOrg.findMany.mockResolvedValueOnce([{ orgNodeId: 'org-1' }]);
    mockPrisma.knowledgeBase.findMany.mockResolvedValueOnce([{ id: 'kb-org-1' }]);

    // 3. Mock 行业库授权
    mockPrisma.industryGrant.findMany.mockResolvedValueOnce([{ kbId: 'kb-industry-1' }]);
  ...
  it('resolves industry organization grants through the member organization and all ancestors', async () => {
    ...
    mockPrisma.kbAdmin.findMany.mockResolvedValue([]);
    mockPrisma.industryGrant.findMany.mockResolvedValue([{ kbId: 'kb-industry-1' }]);
    ...
    expect(visibleKbs).toContain('kb-industry-1');
    expect(grantSubjects).toEqual(expect.arrayContaining([
  ...
  it('keeps industry module access from the industry administrator role', async () => {
    mockPrisma.userRole.findMany.mockResolvedValue([
      { role: { permissions: ['chat.use', 'kb.read', 'reader.read', 'kb.industry.read', 'kb.industry.grant'] } },
    ]);
    const capabilities = await service.getCapabilities('user-industry-admin');
    expect(capabilities).toEqual(expect.arrayContaining(['kb.industry.read', 'kb.industry.grant']));
  });
  ...
  it('does not infer organization management from an OrgAdmin row without the role', async () => {
    mockPrisma.userRole.findMany.mockResolvedValue([{ role: { permissions: ['chat.use', 'kb.read', 'reader.read'] } }]);
